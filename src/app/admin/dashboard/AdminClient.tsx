...
export default function AdminClient() {
  ...
  const load = useCallback(async () => {
    ...
    // Fetched separately: if this fails, it should never blank out the
    // accounts list above, which may have loaded fine.
    try {
      const batchRes = await fetch("/api/batch-settings");
      const batchData = await batchRes.json();
      if (batchRes.ok) setBatches(batchData.batches || []);
    } catch {
      // Batch-year picker just won't show; not fatal to this page.
    }
  }, []);
