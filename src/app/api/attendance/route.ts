...
import { SECTIONS } from "@/lib/batches";
import { getActiveBatchKeys } from "@/lib/batchSettings";
import { resolveBatchAccess, classRepSection } from "@/lib/attendanceAccess";
...
export async function GET(req: Request) {
  ...
  let section = searchParams.get("section"); // "PN1" | "PN2" | null (both, for SW/admin)
  ...
  // A class rep is locked to their own class; social workers/admins may filter by
  // section via the query param, or omit it to see both PN1 and PN2 combined.
  const repSection = classRepSection(session);
  if (repSection) section = repSection;

  if (!batch || !(await getActiveBatchKeys()).includes(batch)) {
  ...
  const students = await prisma.student.findMany({
    where: { batch: batch as any, ...(section ? { section: section as any } : {}) },
    orderBy: { fullName: "asc" },
    select: { id: true, fullName: true, isClassRep: true, section: true },
  });
  ...
  return NextResponse.json({ students, records, monthStart: start.toISOString(), batch, section });
}
...
export async function POST(req: Request) {
  ...
  const repSection = classRepSection(session);
  if (repSection && student.section !== repSection) {
    return NextResponse.json({ error: "Student is outside your class." }, { status: 403 });
  }
