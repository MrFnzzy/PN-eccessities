...
export async function POST(req: Request) {
  ...
  // Each batch has two classes (PN1 and PN2), each with its own rep. Clear any
  // existing rep for this student's specific class only — not the whole batch.
  await prisma.student.updateMany({
    where: { batch: socialWorker.batch, section: student.section, isClassRep: true },
    data: { isClassRep: false },
  });
