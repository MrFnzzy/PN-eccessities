...
import { BATCHES, BATCH_LABEL, BatchKey, SECTIONS, SECTION_LABEL, SectionKey } from "@/lib/batches";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ fullName, email, password, batch, section }),
      });
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-10">
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
        ...
        <div>
          <label className="mb-1 block text-sm font-semibold text-ink/80">Class</label>
          <div className="grid grid-cols-2 gap-2">
            {SECTIONS.map((s) => (
              <button
                ...
                className={`rounded-xl border px-3.5 py-2.5 text-center text-sm font-semibold transition-colors ${
                  section === s
                    ? "border-sky bg-sky/10 text-sky-deep"
                    : "border-ink/15 text-ink/70 hover:border-sky"
                }`}
