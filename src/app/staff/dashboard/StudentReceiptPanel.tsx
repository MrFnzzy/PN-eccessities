...
export default function StudentReceiptPanel({
  ...
  async function handleConfirm(id: string) {
    setBusyId(id);
    setNotice("");
    const res = await fetch(`/api/receipts/detail?id=${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete(id: string, periodLabel: string) {
    ...
    setBusyId(id);
    setNotice("");
    const res = await fetch(`/api/receipts/detail?id=${id}`, { method: "DELETE" });
    setBusyId(null);
    if (!res.ok) {
      play("error");
  ...
  async function handleDownloadAck(id: string) {
    ...
    setNotice("");
    try {
      const res = await fetch(`/api/receipts/detail/acknowledgement?id=${id}`);
      if (!res.ok) {
